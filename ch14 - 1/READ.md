...
인자는 함수를 호출할 때, 매개변수에 실제로 전달하는 값이고,
매개변수는 함수 내부에서 전달된 값을 저장하고 사용하는 역할입니다.



함수가 호출될 때 컴퓨터에 의해 자동으로 실행되는 2가지를 설명하라.

매개변수가 메모리에할당되고 인자값으로 초기화 됩니다.
...
다른 함수에서 선언된 지역변수의 값을 변경하는 것이 가능합니다.
주소로 다른 함수의 변수 접근(간접 참조)은 가능합니다.
...
과제2번의 함수를 값에 의한 호출을 이용하여 2를 증가시킨 후 리턴하고 
